...
#for qss file,conytain syntax of qt customize sheet for chosen graphic component
from styles import *
...
class Ui(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        # main window parameters and main window=self
        self.setObjectName("MainWindow")
        self.resize(550, 600)
        self.setMinimumSize(QtCore.QSize(450, 600))
        self.setWindowFlags(QtCore.Qt.CustomizeWindowHint)
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap("calculator-48_45851.ico"), QtGui.QIcon.Normal, QtGui.QIcon.On)
        self.setWindowIcon(icon)
        self.setStyleSheet(style.mainWindow)
        self.centralwidget = QtWidgets.QWidget(self)

        #gui coding start with widgets
        ...
        # backspace button
        self.bt_backspace.clicked.connect(lambda: self.clrClick())

        # buttons for operators
        self.bt_div.clicked.connect(lambda: self.setoprts('÷'))
        self.bt_multiply.clicked.connect(lambda: self.setoprts('x'))
        self.bt_minus.clicked.connect(lambda: self.setoprts('-'))
        self.bt_plus.clicked.connect(lambda: self.setoprts('+'))

        # button for equal
        self.bt_equal.clicked.connect(lambda: self.rtrnval())
        ...
        def showCredits():
            ...
            QtCore.QTimer.singleShot(2100, lambda: self.lineEdit_values.setPlaceholderText("BHARAT"))
            QtCore.QTimer.singleShot(3500, lambda: self.lineEdit_values.setPlaceholderText("GURSIMRANJIT"))
            QtCore.QTimer.singleShot(4500, lambda: self.lineEdit_values.setPlaceholderText("SHELLY"))
            QtCore.QTimer.singleShot(5500, lambda: self.lineEdit_values.setPlaceholderText("RIDHAM"))
            QtCore.QTimer.singleShot(7500, lambda: self.lineEdit_values.setPlaceholderText(" "))
            QtCore.QTimer.singleShot(8100, lambda: self.bt_copy.setEnabled(True))

        self.bt_copy.clicked.connect(lambda: showCredits())

        # key pressed
        self.lineEdit_values.Enterkeyprs = self.Enterkeyprs
        ...
        # write number or print numberic values on main frame window
    def retranslateUi(self, MainWindow):
        ...
        self.label_credits.setText(QCoreApplication.translate("MainWindow", u"Modern Calculator                                                        created by-Bharat,Gursimranjit,Shelly,Ridham", None))

    #creation of App starting
    # resize function
    def resizing(self, event):
        self.someFunction()
        return super(Ui, self).resizing(event)
